...
<head>
  ...
  <title>Evalexa | Student Register</title>
...
<body class="bg-gradient-to-b from-purple-400 via-blue-300 to-white text-gray-900 min-h-screen flex items-center justify-center">
  ...
  <!-- Register Card -->
  <div class="bg-gray-100 rounded-2xl shadow-xl my-16 p-8 w-full max-w-lg space-y-6 border border-gray-400">

    <!-- Logo + Title -->
    <div class="text-center">
      <i class="fas fa-user-graduate text-purple-600 text-4xl mb-2"></i>
      <h1 class="text-2xl font-bold text-purple-600">Evalexa</h1>
      <p class="text-sm text-gray-700 mt-1">Student Registration</p>
    </div>
    ...
    <c:if test="${not empty success}">
      <div class="text-sm font-medium px-4 py-3 rounded border bg-purple-900/25 text-purple-500 border-purple-600">
    ...
    <form action="${pageContext.request.contextPath}/student/register" method="POST" class="space-y-5">
      ...
      <div>
        ...
        <input type="text" id="full_name" name="full_name" required placeholder="Enter full name"
               class="w-full px-4 py-2 bg-gray-200 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500" />
      </div>
      ...
      <div>
        <button id="submitBtn" type="submit" disabled
                class="w-full py-2 px-4 bg-purple-600 text-white font-semibold rounded-lg transition opacity-50 cursor-not-allowed">
    ...
    <div class="text-center text-xs text-gray-500 pt-4 border-t border-gray-400">
      Already have an account? <a href="${pageContext.request.contextPath}/student/login" class="text-purple-600 hover:underline">Login</a>
    </div>
  </div>
  ...
  <script>
    ...
    function validatePassword() {
      ...
      lengthHint.className = hasLength ? "text-purple-500" : "text-red-500";
      uppercaseHint.className = hasUpper ? "text-purple-500" : "text-red-500";
      lowercaseHint.className = hasLower ? "text-purple-500" : "text-red-500";
      digitHint.className = hasDigit ? "text-purple-500" : "text-red-500";

      const isValid = hasLength && hasUpper && hasLower && hasDigit;
      submitBtn.disabled = !isValid;
      submitBtn.classList.toggle("opacity-50", !isValid);
      submitBtn.classList.toggle("cursor-not-allowed", !isValid);
      submitBtn.classList.toggle("hover:bg-purple-500", isValid);
    }
